...
function WorkVisual({ id }: { id: string }) {
  ...
  return <div className="work-visual company-visual" aria-label="Illustration of connected sales, payments, customer and inventory records">
    <div className="visual-topline"><span>THE EVERYDAY OPERATING SYSTEM</span><span>In use</span></div>
    <div className="workflow-list">{["Record the sale", "Track payments & customers", "Review bottle stock"].map((label, i) => <div key={label}><span>0{i + 1}</span>{label}<span aria-hidden="true">{i === 2 ? "↺" : "↓"}</span></div>)}</div>
  </div>;
}
  ...
  return (
    <>
      ...
      <section id="services" className="portfolio-section portfolio-container" aria-labelledby="services-heading">
        ...
        <div className="services-grid">{services.map((service) => <article className="service-card" key={service.number}><div className="service-top"><ServiceIcon type={service.icon} /><span>{service.number}</span></div><h3>{service.title}</h3><p>{service.description}</p><div className="service-tools"><h4>Tools & resources</h4><ul>{service.tools.map((tool) => <li key={tool}>{tool}</li>)}</ul></div><div className="service-deliverable">{service.deliverable}</div></article>)}</div>
